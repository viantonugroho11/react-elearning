...
function EditWaliKelas() {
  ...
  //method getDataWalikelasbyid
  const getDataWalikelasbyid = async () => {
    //set axios header dengan type Authorization + Bearer token
    axios.defaults.headers.common['Authorization'] = `Bearer ${token}`
    const respone = await axios.get(`http://appsiaksd.ugcorpusskkni.online/api/admin/walikelas/${id}`)
    //set data walikelas
  ...
  const storePost = async (e) => {
    ...
    axios.defaults.headers.common['Authorization'] = `Bearer ${token}`
    //send data to server
    await axios.put(`http://appsiaksd.ugcorpusskkni.online/api/admin/walikelas/${id}`, formData)
      .then(() => {
