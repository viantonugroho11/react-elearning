...
function EditSiswa() {
  ...
  const getDataSiswaById = async () => {
    ...
    const response = await axios.get(`http://appsiaksd.ugcorpusskkni.online/api/admin/siswa/${id}`);
    //get response data
    ...
    setNisn(data.nisn);
    setKelas(data.kelas);
    setTmpLahir(data.tempat_lahir);
    setTglLahir(data.tgl_lahir);

    setTglMasuk(data.tanggal_masuk_sekolah);
    setUsia(data.usia);
    setJkelamin(data.jenis_kelamin);
    setStatus(data.status);
    setNmAyah(data.nama_ayah);
    setPkAyah(data.pekerjaan_ayah);
    setNmIbu(data.nama_ibu);
    setPkIbu(data.pekerjaan_ibu);
    setAlamat(data.alamat);
    setNoTelp(data.nomor_telepon_orangtua);
    setEmail(data.email);
